...
# Hliang, 2021-Dec-12, Added track class; modified CD classes 
# Hliang, 2021-Dec-14, Modified code with TestHarness
#------------------------------------------#
...
class Track():
    ...
    def position(self, val1):
        if type(val1) == int and val1 >= 1:
            self.__position = val1
        else:
            raise Exception('Expecting the position to be an integer >= 1')
    ...
    def title(self, val2):
        if isinstance(val2, str):
            ...
            raise Exception("Excepting the title to be a string")
    ...
    @length.setter
    def length(self, val3):
        if isinstance(val3, str):
            self.__length = val3
        else:
            raise Exception('Excepting the length to be a string')

    # -- Methods -- #
    # DONE Add Track class methods
    def __str__(self):
        """Returns Track details as formatted string"""
        return '{:0>2},{}({})'.format(self.position, self.title, self.length)
    
    def get_record(self) -> str:
        """Returns: Track record formatted for saving to file"""
        return self.position, self.title, self.length, '\n'
    ...
    def __sort_tracks(self):
        ...
        for track in self.__tracks:
            if (track is not None) and (n < track.position):
                n = track.position
        tmp_tracks = [None] * n  # fill empty positions with None
        for track in self.__tracks:  # Lab9A
            if track is not None:
                tmp_tracks[track.position - 1] = track # move one position left
