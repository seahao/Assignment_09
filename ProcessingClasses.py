...
# Hliang, 2021-Dec-12, Added code to select CD and add track object
# Hliang, 2021-Dec-14, Modified track_info by using tuple in add_track
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        if isinstance(cd_idx, int):
            for item in table:
                if item.cd_id == cd_idx:
                    return item
                else:
                    raise Exception ('ID is not in list!')
        else:
            print('ID should be an integer >=1')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        trackObj = DC.Track(Position, Title, Length)
