...
# Hliang, 2021-Dec-12, Add code to save and load data for CD and track files
# Hliang, 2021-Dec-14, Modified code to load track data
#------------------------------------------#
        ...
        try:
            with open(file_name_CD, 'w') as cdfile:
                for item in lst_Inventory:
                    cdfile.write(item.get_record())
                    
            # DONE add code to save track data to file
            with open(file_name_Track, 'w') as trackfile:
                for item in lst_Inventory:
                    ID = item.cd_id()
                    tracks = item.cd_tracks()
                    for track in tracks:
                        if track is not None:
                            IDwTrack = '{},{}'.format(ID, track.get_record())
                            trackfile.write(IDwTrack)
